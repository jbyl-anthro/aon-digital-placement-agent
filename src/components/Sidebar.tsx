...
export default function Sidebar({ currentState, visibleLines }: SidebarProps) {
  return (
    <aside className="w-72 min-w-[18rem] h-screen bg-aon-navy text-white flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          <svg width="48" height="24" viewBox="0 0 56 28" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="border-l border-white/20 pl-3">
            <div className="text-sm font-medium leading-tight">Digital Placement</div>
            <div className="text-[11px] text-white/40">Agent</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 py-4 border-b border-white/10">
        <div className="text-[10px] uppercase tracking-[0.15em] text-white/30 font-medium mb-3">Workflow</div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-5 pt-4 pb-2">
          <div className="text-[10px] uppercase tracking-[0.15em] text-white/30 font-medium">System Activity</div>
        </div>
        <div className="flex-1 overflow-y-auto px-4 pb-4 activity-stream">
          {visibleLines.length === 0 ? (
            <div className="text-[11px] text-white/15 mt-2 px-1">Waiting for agent execution…</div>
          ) : (
            <div className="space-y-1.5">
              {visibleLines.map((line, i) => (
                <div key={i} className="animate-fade-in text-[10px] leading-relaxed">
